...
def test():
    env_test = gym.make('Conquer2', render_mode='rgb_array', size = 15)
    dir_path = os.path.dirname(os.path.realpath(__file__))
    env_record = gym.wrappers.RecordVideo(env=env_test, video_folder=dir_path, name_prefix="test-video", episode_trigger=lambda x: x % 2 == 0)
